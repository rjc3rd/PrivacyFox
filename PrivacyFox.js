 ...
 * PrivacyFox.js
 *
 * A single, deduplicated merge of Arkenfox's and Betterfox's hardening
 * prefs, plus a small set of PrivacyFox's own overrides. No layering, no
 * "last file loaded wins" ambiguity: every pref below appears exactly once.
 *
 * How this was built: every pref in both source files was compared key by
 * key. 64 prefs appeared in both files -- all 64 already agreed on the same
 * value, so each was kept once (credited to Arkenfox, the base section
 * below). Betterfox's remaining 51 prefs, which don't overlap with Arkenfox
 * at all, are appended in their own section. PrivacyFox adds a further 2
 * prefs of its own on top (see the final section).
 *
 * Credit and thanks to both real upstream projects this is built from:
...
/*******************************************************************************
 * SECTION: ARKENFOX (base -- https://github.com/arkenfox/user.js)
 ******************************************************************************/
...
/*******************************************************************************
 * SECTION: BETTERFOX ADDITIONS
 * (https://github.com/yokoffing/Betterfox -- only prefs not already covered
 * by Arkenfox above; 64 overlapping prefs were deduplicated out, all of
 * which already agreed with Arkenfox's own value)
 ******************************************************************************/
...
/*******************************************************************************
 * SECTION: PRIVACYFOX OVERRIDES
 * PrivacyFox's own additions on top of Arkenfox + Betterfox.
 ******************************************************************************/

// Turn off Firefox Sync / accounts prompts at the pref level too -- belt and
// suspenders alongside policies.json's DisableFirefoxAccounts.
